...
export default function App() {
  ...
  const [tasks, setTasks] = useState(() => {
    // Используем новый ключ, чтобы браузер 100% загрузил все 50 задач
    const saved = localStorage.getItem('wms_hub_full_50_tasks_v2');
    if (saved) {
  ...
  const [chatMessages, setChatMessages] = useState([
    { role: 'assistant', content: 'Привет! Загружены все 50 задач из вашей таблицы. Я готов помогать с отчетами, аналитикой и планом/фактом!' }
  ]);
  const [inputMessage, setInputMessage] = useState('');
  const [isTyping, setIsTyping] = useState(false);

  useEffect(() => {
    localStorage.setItem('wms_hub_full_50_tasks_v2', JSON.stringify(tasks));
  }, [tasks]);
  ...
  const handleSendMessage = (e) => {
    e.preventDefault();
    // Обработка пустого ввода: если пусто, ничего не делаем, белого экрана не будет
    ...
    setTimeout(() => {
      ...
      if (lower.includes('отчет') || lower.includes('месяц') || lower.includes('итоги')) {
        ...
        reply = `📊 **Аналитика:** Нагрузка распределена между Брянцевым, Голиком, Сухоруковым и Довганем. В этом спринте Брянцев Александр закрыл большинство задач инвентаризации в срок.`;
      } else {
        reply = `🤖 Я проанализировал всю вашу базу из ${tasks.length} задач. Вы можете удалять ненужные задачи, редактировать их или запрашивать отчеты по результатам!`;
      }
                      ...
                      {filteredTasks.map(t => (
                        <tr key={t.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-4 pl-6">
                            ...
                            <div className="text-slate-100 font-medium mt-1 w-48">{t.name}</div>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            {t.dependsOn ? (
                              <span className="text-xs bg-amber-500/10 text-amber-300 border border-amber-500/20 px-2 py-0.5 rounded whitespace-nowrap">
                                ...
                                <div key={idx} className="bg-slate-950/60 p-2 rounded-lg border border-slate-800/80 flex flex-wrap items-center justify-between gap-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <input 
                                      ...
                                      className="bg-slate-900 border border-slate-700 rounded px-1 py-0.5 text-xs text-slate-200 focus:outline-none focus:border-fuchsia-500" 
                                    />
                                  </div>
                                </div>
                              ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded">Завершил: 24 задачи</span>
                  </div>
                  <div className="text-xs text-slate-400 space-y-1">
                    <div>Основной ресурс бэкенда.</div>
                  </div>
                </div>
                <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="font-semibold text-fuchsia-400">Голик Егор (DB)</span>
                    <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded">Завершил: 2 задачи</span>
                  </div>
                  <div className="text-xs text-slate-400 space-y-1">
                    <div>Высокая скорость выполнения запросов БД.</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col h-[600px]">
              <div className="pb-4 border-b border-slate-800 flex items-center gap-3 shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Спросите про любую из {tasks.length} задач!</p>
                </div>
              </div>
